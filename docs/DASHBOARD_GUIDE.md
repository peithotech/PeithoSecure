...
## 1. 📊 Activity Stream (Observability & Forensics)

The Activity Stream is the home view. It functions like Wireshark for autonomous AI agent tool calls.
...
* **Security Event Stream Table**:
  ...
  * **`PRINCIPAL`**: The agent identity (e.g., `agent.researcher`, `agent.quant_modeler`).
  * **`TOOL`**: The invoked tool function (e.g., `query_database`, `execute_wire_transfer`).
